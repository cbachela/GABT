  ...
  Object <- loadSensor( sensor_name = "bbturbulence", b_new = TRUE )  
  X_bm <- Object$data$X_bm
  X_bm <- X_bm[isWeekday(time(X_bm)), ]
  X_level <- cumulated(X_bm, "discrete")
  signal <- Object$signal$base[ ,"md_delta"]
  dates <- intersect(rownames(X_bm), rownames(signal))
  X_bm <- X_bm[dates, ]
  signal <- signal[dates, ]
  
  
  apply_roll_y <- list( fun = sum,
                        width = 21,
                        by = 1 )
  # debugonce(trainingData)
  # debugonce(weightsFun)
  ...
  ##################
  # Bear Bull Absorption Ration
  
  BBS <- loadSensor( sensor_name = "bbs", b_new = TRUE ) 
  states <- BBS$signal$insample
  BBT <- loadSensor( sensor_name = "bbturbulence", b_new = TRUE ) 
  X <- BBT$data$X
  dates <- intersect(rownames(states), rownames(X))
  states <- states[dates, ]
  X <- X[dates, ]
  covmat_bear <- cov(X[which(states == -1), ])
  covmat_bull <- cov(X[which(states == 1), ])
  E_bear <- eigen(covmat_bear)
  E_bull <- eigen(covmat_bull)
  
  evals <- cbind(E_bear$values, E_bull$values)
  barplot( t(evals), beside = TRUE, col = 1:ncol(evals))  

  apply( evals, 2, function(x) { sum(x[1:5]) / sum(x) } )  
  
  
  FUN <- function(X)
  {
    covmat <- cov.ewma(X, tau = 104)
    evals <- eigen(covmat)$values
    return( evals )
  }
  evals_roll <- applyRoll( Data = X, Width = 252, By = 1, FUN = FUN )

  d_bear <- apply( evals_roll, 1, function(x) { sqrt(sum((x - E_bear$values)^2)) } )
  d_bull <- apply( evals_roll, 1, function(x) { sqrt(sum((x - E_bull$values)^2)) } )
  
  d <- timeSeries( cbind(d_bear = d_bear, d_bull = d_bull), 
                   time(evals_roll) )
  
  plot( d, plot.type = "single" )  
  ...
  # Distance to eigenvectors
  FUN <- function(X)
  {
    covmat <- cov.ewma(X, tau = 104)
    evecs <- eigen(covmat)$vectors
    # d1 <- sqrt( sum( (evecs - E_bear$vectors)^2 ) )
    # d2 <- sqrt( sum( (evecs - E_bull$vectors)^2 ) )
    d1 <- sum( abs(evecs - E_bear$vectors) )
    d2 <- sum( abs(evecs - E_bull$vectors) )
    ans <- setNames( c(d1, d2), c("bear", "bull") )
    return( ans )
  }
  DV <- applyRoll( Data = X, Width = 252, By = 1, FUN = FUN )
  
  plot( DV, plot.type = "single" )
  plot( ema(DV, 0.01), plot.type = "single" )
